...
// ChatScreen.jsx — Main chat screen with Android/iOS keyboard handling
...
export default function ChatScreen({ navigation }) {
  ...
  const handleSend = async (text) => {
    ...
    // FI9_NAYEK v14 ANDROID FIX: Ajouter message utilisateur immédiatement AVANT l'appel API
    // Le message doit être visible tout de suite, sans attendre la réponse API
    const userMessage = {
    ...
    try {
      ...
      // FI9_NAYEK v14 ANDROID FIX: Préserver TOUT l'historique - merge intelligent SANS écrasement
      // Le message utilisateur a déjà été ajouté, on merge juste la réponse API
      setMessages((prev) => {
        // Garder tous les messages existants (le message utilisateur est déjà là, pas de pending)
        const history = prev;
      ...
      setMessages((prev) => {
        // FI9_NAYEK v14 ANDROID FIX: Le message utilisateur est déjà dans prev, on ajoute juste l'erreur
          ...
          <FlatList
            ...
            contentContainerStyle={
              messages.length === 0 
                ...
                : [styles.messageListContainer, { paddingBottom: 200 }]
            }
